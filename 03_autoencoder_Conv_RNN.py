...
# Training Model
input = K.layers.Input(shape=(timesteps - channels - 1, channels))
layer = K.layers.SimpleRNN(hneurons, activation='tanh', return_sequences=True, dropout=0.01)(input) ## <<<<<<<<<<< Here
dense = K.layers.Dense(1, activation='tanh')
...
# Prediction Model
input = K.layers.Input(shape=(1, channels))
layer = K.layers.SimpleRNN(hneurons, activation='tanh', return_sequences=True)(input) ## <<<<<<<<<<< Here
dense = K.layers.Dense(1, activation='tanh')
layer = K.layers.TimeDistributed(dense)(layer)
layer = K.layers.Dense(75, activation='tanh')(layer)
layer = K.layers.Dense(75, activation='tanh')(layer)
...
P_model = K.models.Model(input, output)
K.utils.print_summary(model)
